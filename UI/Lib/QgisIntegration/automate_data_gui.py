...
#import paths
input_path = "C:/Users/MaxWillx/CMU course/Paws Project/gui_qgis/Quick_Employment/QuickEmployment_Toy/auto_input/"
output_path_shapefiles = "C:/Users/MaxWillx/CMU course/Paws Project/gui_qgis/Quick_Employment/QuickEmployment_Toy/shapefiles/"
output_path_excel = "C:/Users/MaxWillx/CMU course/Paws Project/gui_qgis/Quick_Employment/QuickEmployment_Toy/csv_output/"

# specify layers we want to find distances from
dist_layers = ['toy_patrol.shp', 'toy_poaching.shp',
               'toy_road.shp', 'toy_river.shp']
...
def mmqgis_attribute_export(qgis, outfilename, layername, attribute_names, field_delimiter, line_terminator, decimal_mark):
  ...
  if (not attribute_names) or (len(attribute_names) <= 0):
    ...
    attribute_names = [layer.attributeDisplayName(
        x) for x in attribute_indices]
  ...
  writer = csv.writer(outfile, delimiter=field_delimiter,
  ...
  for index, feature in enumerate(layer.getFeatures()):
    if (index % 50) == 0:
      qgis.mainWindow().statusBar().showMessage \
          ("Exporting feature " + unicode(feature.id()) +
           " of " + unicode(feature_count))
    attributes = feature.attributes()

    row = []
    for column in attribute_indices:
      # print unicode(column) + " (" + decimal_mark + "): " +
      # type(attributes[column]).__name__
...
# only use this class for creating discretized grid, doesn't work for
# centroids layer
...
conservation_site = import_layer(
    input_path + boundary_file, "conservation site")
...
site_layer = QgsVectorLayer(
    input_path + boundary_file, "conservation site", "ogr")
overlayAnalyzer = qgis.analysis.QgsOverlayAnalyzer()
overlayAnalyzer.intersection(
    grid.get_layer(), site_layer, output_path_shapefiles + "discretized_grid.shp")

# create DN attribute to uniquely identify grid cells
grid = import_layer(output_path_shapefiles +
                    "discretized_grid.shp", "temp grid")
grid.startEditing()
grid.dataProvider().addAttributes([QgsField("DN", QVariant.Int)])
grid.updateFields()

# enumerate DN based on ordering of x and y coordinates
pos_id_list = [(feat.geometry().asPoint().x(), feat.geometry(
).asPoint().y(), feat.id()) for feat in grid.getFeatures()]
pos_id_list.sort()
...
points.startEditing()
points.dataProvider().addAttributes([QgsField("DN", QVariant.Int), QgsField(
    "X", QVariant.Double), QgsField("Y", QVariant.Double)])
points.updateFields()
...
QgsVectorFileWriter.writeAsVectorFormat(
    points, output_path_shapefiles + "point_grid.shp", "utf-8", None, "ESRI Shapefile")


# load the centroids and grid
centroids = import_layer(output_path_shapefiles +
                         "point_grid.shp", "point grid")
grid = import_layer(output_path_shapefiles +
                    "discretized_grid.shp", "discretized grid")
...
for int_layer in int_layers:
  ...
  for line_feature in cur_layer.getFeatures():
    cands = grid.getFeatures(QgsFeatureRequest().setFilterRect(
        line_feature.geometry().boundingBox()))
  ...
  res = QgsVectorFileWriter.writeAsVectorFormat(
      layer, output_path_shapefiles + "(1)is-" + int_layer, "utf-8", layer.crs(), "ESRI Shapefile", 1)
  ...
  layer = iface.activeLayer()
  res = QgsVectorFileWriter.writeAsVectorFormat(
      layer, output_path_shapefiles + "(0)is-" + int_layer, "utf-8", layer.crs(), "ESRI Shapefile", 1)
  ...
  final_layer = import_layer(
      output_path_shapefiles + "is-" + int_layer, "is-" + int_layer)

  # export as csv
  mmqgis_attribute_export(iface,
                          output_path_excel + "is-" +
                          int_layer[0: (len(int_layer) - 4)] + ".csv",
...
for dist_layer in dist_layers:
  ...
  worker = Worker(centroids, cur_layer, "dist-" + name, "join_",
                  selectedinputonly=False, selectedjoinonly=False)
  worker.run()

  # save the distance layer
  res = QgsVectorFileWriter.writeAsVectorFormat(
      worker.mem_joinl, output_path_shapefiles + "dist-" + dist_layer, "utf-8", None, "ESRI Shapefile")
  if res != QgsVectorFileWriter.NoError:
  ...
  actual_layer = import_layer(
      output_path_shapefiles + "dist-" + dist_layer, name)

  # export as csv
  mmqgis_attribute_export(iface,
                          output_path_excel + "dist-" +
                          dist_layer[0: (len(dist_layer) - 4)] + ".csv",
...
for raster_name in raster_layers:
  ...
  centroids.dataProvider().addAttributes(
      [QgsField(feature_name, QVariant.Double)])
  ...
  for feat in features:
    ...
    raster_value = raster_provider.identify(
    ...
    for field in fields:
      if field.name() != feature_name:
        old_fields[field.name()] = feat.attributes()[
            centroids.fieldNameIndex(field.name())]
  ...
  mmqgis_attribute_export(iface,
                          output_path_excel +
                          raster_name[0: (len(raster_name) - 4)] + ".csv",
...
for patrol in patrols:
  ...
  layer = points
  res = QgsVectorFileWriter.writeAsVectorFormat(
      layer, export_points, "utf-8", None, "ESRI Shapefile")
  if res != QgsVectorFileWriter.NoError:
  ...
  processing.runalg("qgis:countpointsinpolygon", input_path +
                    "discretized_grid.shp", export_points, 'NUMPOINTS', Result)
  ...
  # export as csv
  mmqgis_attribute_export(iface,
                          output_path_excel + "num" +
                          patrol[0:(len(patrol) - 4)] + ".csv",
...
# remember to print out xmin, ymin, xmax, ymax
